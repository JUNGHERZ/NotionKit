...
},
{
  ...
  desc: {
    en: 'A decorative band above the page, 200px tall – the token <code>--nk-cover-height</code>, on <code>:root</code> or on one cover; Notion\'s own takes about 30vh, <code>clamp(200px, 30vh, 300px)</code> comes close. Three radial gradients mixed from the <code>--nk-decor-*</code> tokens over <code>--nk-bg-callout</code>. For a picture put an <code>&lt;img&gt;</code> inside: it fills the band and is cropped, never stretched, and <code>object-position</code> moves the crop, as Notion\'s “Reposition” does. The gradient stays underneath while it loads.',
    de: 'Ein Zierband über der Seite, 200px hoch – das Token <code>--nk-cover-height</code>, auf <code>:root</code> oder an einem Cover; Notions eigenes nimmt etwa 30vh, <code>clamp(200px, 30vh, 300px)</code> kommt nahe heran. Drei radiale Verläufe, gemischt aus den <code>--nk-decor-*</code>-Tokens über <code>--nk-bg-callout</code>. Für ein Bild ein <code>&lt;img&gt;</code> hineinsetzen: Es füllt das Band und wird beschnitten, nie verzerrt, und <code>object-position</code> verschiebt den Ausschnitt wie Notions „Neu positionieren“. Der Verlauf bleibt darunter, solange es lädt.',
  },
  mobile: { en: 'Full bleed, as tall as <code>--nk-cover-height</code>; lower the token if the band eats too much of a short screen. Without a page icon the page keeps its top padding under the cover.', de: 'Randlos, so hoch wie <code>--nk-cover-height</code>; das Token senken, wenn das Band zu viel eines kurzen Schirms frisst. Ohne Seiten-Icon behält die Seite ihren oberen Abstand unter dem Cover.' },
...
{
  id: 'nk-progress', group: 'database', classes: ['nk-progress', 'nk-progress-label', 'wide', 'nk-progress-row'],
  title: { en: 'Progress bar', de: 'Fortschrittsbalken' },
  desc: {
    en: 'A 6px rail whose fill is an <code>&lt;i&gt;</code> with a percentage width. Track and fill use Notion\'s blue tint and mid-tone, so a re-theme carries them along. 110px wide in a cell; <code>wide</code> fills its row – a property value, a panel, a course overview – and in a flex row the label keeps its place beside it; in a flex column it stays a 6px bar. <code>.nk-progress-row</code> holds a bar and its label as one row – in a flex column, a panel, the two would otherwise stand one under the other: the label sits beside the bar, on its middle, and a wide bar fills the rest.',
    de: 'Eine 6px-Schiene, deren Füllung ein <code>&lt;i&gt;</code> mit Prozentbreite ist. Schiene und Füllung nutzen Notions blaue Tönung und den blauen Mittelton und ziehen bei einem Theme-Wechsel mit. 110px breit in einer Zelle; <code>wide</code> füllt seine Zeile – ein Eigenschaftswert, ein Panel, eine Kursübersicht – und in einer flex-Zeile behält das Label seinen Platz daneben; in einer flex-Spalte bleibt er ein 6px-Balken. <code>.nk-progress-row</code> hält Balken und Label als eine Zeile – in einer flex-Spalte, einem Panel, stünden die beiden sonst untereinander: Das Label steht neben dem Balken, auf seiner Mitte, und ein breiter Balken füllt den Rest.',
  },
  ...
  html: W => `<span class="nk-progress"><i style="width:65%"></i></span><span class="nk-progress-label">65 %</span><br><br>
...
<div class="nk-panel" style="max-width:360px;margin-top:14px"><h3>${W.progress}</h3><span class="nk-progress-row"><span class="nk-progress wide"><i style="width:40%"></i></span><span class="nk-progress-label">40 %</span></span></div>`,
},
{
...
{
  id: 'nk-list', group: 'database', classes: ['nk-list', 'nk-list-item', 'l-icon', 'l-title', 'l-meta', 'last', 'static'],
  title: { en: 'List view', de: 'Listen-Ansicht' },
  desc: {
    en: 'The third database view: one line per row – icon, title and a few properties on the right – as Notion shows a database under “List”. Hairlines between rows and the hover wash across the whole row, like a table without columns; the last row drops its line. As an <code>&lt;a&gt;</code> each row is a real link. Put it in <code>.nk-database</code> under the view tabs, or use it on its own for any list of pages. A row that is no target itself – a file with its own Remove button – takes <code>.static</code>: no hand, no hover wash.',
    de: 'Die dritte Datenbank-Ansicht: eine Zeile pro Eintrag – Icon, Titel und rechts ein paar Eigenschaften –, wie Notion eine Datenbank unter „Liste“ zeigt. Haarlinien zwischen den Zeilen und der Hover-Hauch über die ganze Zeile, wie eine Tabelle ohne Spalten; die letzte Zeile verliert ihre Linie. Als <code>&lt;a&gt;</code> ist jede Zeile ein echter Link. In <code>.nk-database</code> unter die View-Reiter setzen oder allein für jede Liste von Seiten nutzen. Eine Zeile, die selbst kein Ziel ist – eine Datei mit eigenem Entfernen-Knopf –, bekommt <code>.static</code>: keine Hand, kein Hover-Hauch.',
  },
...
// ============================================================ 5.6 FORMS
{
  ...
  desc: {
    en: 'One shared shape for all three: filled with <code>--nk-bg-input</code> inside a hairline, 32px tall at 14px — Notion\'s input, not an outlined white box. The focus ring is mixed from the accent, so it re-brands with it. <code>.wide</code> makes an input fill its row. Each keeps at least 210px, except inside a panel, where it never runs past the edge of a narrow tile; a field row keeps the 210px there too.',
    de: 'Eine gemeinsame Form für alle drei: gefüllt mit <code>--nk-bg-input</code> in einer Haarlinie, 32px hoch bei 14px – Notions Input, keine umrandete weiße Box. Der Fokusring wird aus dem Akzent gemischt und färbt sich mit ihm um. <code>.wide</code> füllt die Zeile. Jedes behält mindestens 210px, außer in einem Panel, wo es nie über den Rand einer schmalen Kachel hinausragt; eine Feldzeile behält die 210px auch dort.',
  },
...
{
  id: 'nk-gallery', group: 'database', classes: ['nk-gallery', 'small', 'large', 'fit', 'card-item'],
  title: { en: 'Gallery view', de: 'Galerie-Ansicht' },
  desc: {
    en: 'The fifth database view: the rows as cards with a picture on top, in a grid that fills the row – Notion\'s gallery, for a course catalog or a reading list. A card is the board\'s <code>.nk-card</code> with <code>.card-title</code> and <code>.card-meta</code>; a <code>.nk-cover</code> as its first child runs to the card\'s edges in 2:1, a picture inside cropped to fill – without one the cover gradient shows. <code>.small</code> and <code>.large</code> set the card size, columns from 180px and 340px instead of 260px; <code>.fit</code> shows a picture whole, for logos. A <code>.nk-new-row</code> at the end becomes the add card. Clicking a card is your script, as with a row of the table. A card to open in a new tab or to copy the address of is a link, <code>&lt;a class="nk-card" href&gt;</code>, in a <code>.card-item</code> that carries <code>role="listitem"</code> – the link keeps its own role – and the card fills it.',
    de: 'Die fünfte Datenbank-Ansicht: die Einträge als Karten mit einem Bild oben, in einem Raster, das die Zeile füllt – Notions Galerie, für einen Kurskatalog oder eine Leseliste. Eine Karte ist die <code>.nk-card</code> des Boards mit <code>.card-title</code> und <code>.card-meta</code>; ein <code>.nk-cover</code> als erstes Kind läuft in 2:1 bis an die Ränder der Karte, ein Bild darin wird beschnitten – ohne Bild zeigt sich der Cover-Verlauf. <code>.small</code> und <code>.large</code> setzen die Kartengröße, Spalten ab 180px und 340px statt 260px; <code>.fit</code> zeigt ein Bild ganz, für Logos. Ein <code>.nk-new-row</code> am Ende wird zur Hinzufügen-Karte. Der Klick auf eine Karte ist dein Skript, wie bei einer Tabellenzeile. Eine Karte, die sich in einem neuen Tab öffnen und deren Adresse sich kopieren lassen soll, ist ein Link, <code>&lt;a class="nk-card" href&gt;</code>, in einem <code>.card-item</code> mit <code>role="listitem"</code> – der Link behält seine eigene Rolle –, und die Karte füllt es.',
  },
  ...
  <div class="card-item" role="listitem"><a class="nk-card" href="#nk-gallery"><div class="nk-cover"></div><div class="card-title">📣 ${W.launch}</div><div class="card-meta"><span class="nk-tag yellow">${W.planned}</span><span>📅 02.06.2026</span></div></a></div>
  <div class="nk-new-row" role="button" tabindex="0">${W.newPage}</div>
</div>`,
},
{
  id: 'nk-copy-field', group: 'forms', classes: ['nk-copy-field', 'cf-value', 'cf-btn', 'copied', 'mono', 'wrap', 'wide', 'icons'],
  title: { en: 'Copy field', de: 'Kopierfeld' },
  desc: {
    en: 'A value to take along – a link to share, an address, a key – in a field of its own, as tall as an input, with its actions inside on the right. <code>.cf-value</code> holds the text on one line with an ellipsis, and a click selects it whole; <code>.cf-btn</code>s are the quiet actions – Copy and, for a secret, Show. <code>.copied</code> on a button is the moment after, in green; the copying is your script. <code>mono</code> for addresses, keys and code, <code>wrap</code> lets a long value break with the actions on its first line, <code>wide</code> fills the row. Like an input it keeps at least 210px, except inside a panel. A button may carry an <code>&lt;svg&gt;</code> beside its word in a <code>&lt;span&gt;</code> – two overlapping squares for Copy, an eye for Show, a check for the moment after – and the word as <code>aria-label</code>: on a phone, and anywhere with <code>icons</code>, the icon stands in for the word, as in Notion, and a masked key keeps a dozen characters more.',
    de: 'Ein Wert zum Mitnehmen – ein Link zum Teilen, eine Adresse, ein Schlüssel – in einem eigenen Feld, so hoch wie ein Eingabefeld, mit seinen Aktionen rechts darin. <code>.cf-value</code> hält den Text in einer Zeile mit Auslassungspunkten, ein Klick markiert ihn ganz; <code>.cf-btn</code>s sind die leisen Aktionen – Kopieren und, bei einem Geheimnis, Zeigen. <code>.copied</code> an einem Button ist der Moment danach, in Grün; das Kopieren übernimmt dein Skript. <code>mono</code> für Adressen, Schlüssel und Code, <code>wrap</code> lässt einen langen Wert umbrechen, die Aktionen bleiben in der ersten Zeile, <code>wide</code> füllt die Zeile. Wie ein Input behält es mindestens 210px, außer in einem Panel. Ein Button kann neben seinem Wort in einem <code>&lt;span&gt;</code> ein <code>&lt;svg&gt;</code> tragen – zwei überlappende Quadrate für Kopieren, ein Auge für Zeigen, ein Haken für den Moment danach – und das Wort als <code>aria-label</code>: Auf dem Telefon und überall mit <code>icons</code> steht das Symbol für das Wort, wie in Notion, und ein verdeckter Schlüssel behält ein Dutzend Zeichen mehr.',
  },
  mobile: { en: 'Keeps to its column: the value is cut, never the actions. Buttons that carry an icon show it instead of their word.', de: 'Bleibt in seiner Spalte: Gekürzt wird der Wert, nie die Aktionen. Buttons mit einem Symbol zeigen es statt ihres Worts.' },
...
<div class="nk-copy-field mono icons" style="max-width:340px;margin-top:10px"><span class="cf-value">ntn_••••••••••••••••••••</span><button class="cf-btn" aria-label="${W.show}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg><span>${W.show}</span></button><button class="cf-btn" aria-label="${W.copy}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 9V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h4"/><rect x="9" y="9" width="12" height="12" rx="2"/></svg><span>${W.copy}</span></button><button class="cf-btn copied" aria-label="${W.copied}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg><span>${W.copied}</span></button></div>`,
...
},
{
  ...
  desc: {
    en: 'Five variants, 28px tall at 14px. <code>.secondary</code> and <code>.danger</code> are Notion\'s white button: the outline is <code>--nk-shadow-btn</code>, a 1px inset ring plus a 1px drop, not a border. Hover is an opacity shift on the filled ones and a wash on the white ones — never a hue change. <code>.small</code> (24px) combines with any variant. Stretched across a column – a full-width button on a sign-in page – it keeps its label centred.',
    de: 'Fünf Varianten, 28px hoch bei 14px. <code>.secondary</code> und <code>.danger</code> sind Notions weißer Button: die Kontur ist <code>--nk-shadow-btn</code>, ein 1px-Innenring plus 1px Schatten, kein Rahmen. Hover ist bei gefüllten Buttons eine Deckkraft-Änderung, bei weißen ein Hauch – nie ein Farbwechsel. <code>.small</code> (24px) lässt sich mit jeder Variante kombinieren. Über eine Spalte gestreckt – ein Button über die volle Breite auf einer Anmeldeseite – bleibt seine Beschriftung mittig.',
  },
...
},
{
  ...
  desc: {
    en: 'Steps through a short flow – connecting an account, setting up a model – calm and vertical: a numbered circle per step joined by a hairline, done steps on the green tag with a check, the current one ringed in the accent and marked <code>aria-current="step"</code>, the rest quiet. <code>.st-desc</code> adds a line under a step. The states go per step, in any order: a step that was skipped is <code>.skipped</code> – a dashed ring around a dash – while a later one is <code>.done</code>. The label as a <code>&lt;button class="st-label"&gt;</code> makes a step one to jump to; as an <code>&lt;a class="st-label" href&gt;</code> one to open in a new tab or to copy the address of, looking the same. <code>.horizontal</code> sets the steps in one row above a wizard, joined by lines. Not a Notion block; in Notion\'s idiom it is a numbered list with checks.',
    de: 'Schritte durch einen kurzen Ablauf – ein Konto verbinden, ein Modell einrichten –, ruhig und vertikal: ein nummerierter Kreis pro Schritt, verbunden durch eine Haarlinie, erledigte Schritte mit Haken auf dem grünen Tag, der aktuelle im Akzent umrandet und mit <code>aria-current="step"</code> markiert, der Rest leise. <code>.st-desc</code> ergänzt eine Zeile unter einem Schritt. Die Zustände gelten je Schritt, in beliebiger Folge: Ein übersprungener Schritt ist <code>.skipped</code> – ein gestrichelter Ring um einen Strich –, während ein späterer <code>.done</code> ist. Das Label als <code>&lt;button class="st-label"&gt;</code> macht einen Schritt anspringbar; als <code>&lt;a class="st-label" href&gt;</code> lässt er sich in einem neuen Tab öffnen und seine Adresse kopieren, bei gleichem Aussehen. <code>.horizontal</code> setzt die Schritte in eine Zeile über einem Assistenten, verbunden durch Linien. Kein Notion-Block; in Notions Sprache ist es eine nummerierte Liste mit Haken.',
  },
