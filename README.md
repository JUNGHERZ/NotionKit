<p align="center">
  <img src="https://img.shields.io/badge/version-1.19.0-2383e2?style=flat-square" alt="Version">
  ...
  <img src="https://img.shields.io/badge/gzip-15.1%20KB-d9730d?style=flat-square" alt="Size">
  <img src="https://img.shields.io/badge/license-MIT-lightgrey?style=flat-square" alt="License">
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-v1.19.0-9065b0?style=flat-square" alt="Changelog"></a>
...
- 🪶 **Lightweight** – 131 KB raw / 83 KB minified / 15.1 KB gzipped, no dependencies. The budget is 18 KB gzipped – up to 1.9.0 it was 14 KB, what a new connection delivers in its first round trip; CI keeps to it
...
```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@jungherz-de/notionkit@1.19.0/notionkit.min.css">
```
...
| `nk-sidebar-head`, `nk-sidebar-collapse`, `nk-sidebar` / `nk-sidebar-toggle` (`collapsed`) | On the desktop Notion's « collapses the sidebar, the ☰ brings it back |
...
| `nk-prose` | Rendered Markdown and saved editor HTML – the editor adapter's rules, one class on the container |
...
| `nk-table-wrap`, `nk-table` (`th-icon`, `row-title`, `date-cell`, `person-cell`, `num`, `row-actions`, `th.actions`, `td-text`, `td-desc`), `nk-new-row` | Horizontally scrolling table with add row, right-aligned numbers, buttons per row and text with a tone and a second line |
...
| `nk-gallery` (`small`, `large`, `fit`) with `nk-card` and `nk-cover` | Gallery view: the rows as cards with a picture on top, in three sizes |
...
| `nk-peek-backdrop` (`open`), `nk-peek` (`pk-resize`, `pk-bar`, `pk-body`), `nk-app.peek-inset` | Side peek: a row beside the table, resizable, optionally insetting the page; a bottom sheet on a phone |
| `nk-dialog-backdrop` (`open`), `nk-dialog` (`wide`, `dl-title`, `dl-body`, `dl-actions`) | Dialog for a question or a short form; a bottom sheet on a phone |
| `nk-tooltip` (`open`, `tt-key`) | Hover hint with an optional shortcut |
...
| `nk-toast` (`show`) | Toast |
| `nk-gallery-grid`, `nk-g-item`, `nk-tabs` (`scroll`), `nk-tab`, `nk-tab-panel`, `nk-template-btn` | Gallery grid, tabs – many in a row that scrolls – template buttons |
| `nk-panels` (`flush`), `nk-panel` (`p-head`, `p-end`) | Neutral surfaces in a grid; with cover and page icon a page tile, a tag or button beside the title |
| `nk-steps` (`horizontal`), `nk-step` (`st-mark`, `st-label`, `st-desc`, `done`, `skipped`, `current`) | Calm steps through a short flow, vertical or in a row, states in any order |
...
| `nk-ai-thread`, `nk-ai-msg` (`user`, `bubble`), `nk-ai-actions`, `nk-ai-input-row`, `nk-ai-send` | AI conversation, your own question as a grey bubble |
...
| *(scoped inside the host)* `.ProseMirror`, `.bn-*`, `.novel-*` | Themed overrides for TipTap, BlockNote, Novel; `.ProseMirror` shares every rule with `.nk-prose` |
...
TipTap/ProseMirror is the vanilla-JS path; BlockNote (React) and Novel (Next.js) are covered by the same adapter. What the editor saves shows read-only in `<div class="nk-prose">`: prose and editor read the same rules, so a page looks the same in both – the demo's “Edit · Read” switch proves it to the pixel. The full recipe with slash menu, bubble toolbar and a Notion-style ＋ / ⠿ block handle with drag & drop is [`docs-editor.js`](docs-editor.js) – about 300 lines of plain JavaScript that only put NotionKit classes on their DOM. See it running in the [docs](https://notionkit.jungherz.com/docs.html#editor) and the [demo](https://notionkit.jungherz.com/app.html).
...
├── notionkit.min.css.map    # Its source map, published with it (gitignored)
...
├── covers/                  # Cover pictures of the demo's start view
...
- **eight complete app skeletons** – workspace app, database app, settings modal, AI chat page, form/onboarding page, landing/docs page, home page, sign-in page – each with a "When to use this skeleton?" block
...
| `.open` | `nk-modal-backdrop`, `nk-cmdk-backdrop`, `nk-sheet-backdrop`, `nk-dialog-backdrop`, `nk-peek-backdrop`, `nk-pop.floating`, `nk-tooltip`, `nk-sidebar` (phones), `nk-toggle-arrow` | Overlay visible, peek in, menu or tooltip shown, drawer in / arrow rotated |
| `.floating` `.sheet` | `nk-pop` | Fades in with `.open`; a bottom sheet below 860px |
| `.collapsed` | `nk-tree-children`, `nk-sidebar`, `nk-sidebar-toggle` | Subtree folded; sidebar slid out on the desktop, its ☰ shown |
| `.peek-inset` | `nk-app` | The page moves aside for the open side peek |
...
| `.gray` `.brown` `.orange` `.yellow` `.green` `.blue` `.purple` `.pink` `.red` | `nk-tag`, `nk-avatar` | Notion's nine colours; a tag without one is gray, an avatar the gradient |
| `.small` `.large` `.xlarge` `.square` | `nk-avatar` | 20 / 32 / 56px instead of 24px; workspace-icon corners |
